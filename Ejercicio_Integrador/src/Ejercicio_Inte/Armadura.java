...
public class Armadura {
   ...
   public Armadura(String colorPrimario, String colorSecundario, int nivelDeResistencia, Generador generador) {
    ...
    this.botas[0] = new Bota(10);
    this.botas[1] = new Bota(10);
    this.guantes = new Guante[2];
    this.guantes[0] = new Guante(10);
    this.guantes[1] = new Guante(10);
    this.consola = new Consola(10);
    this.sintetizador = new Sintetizador(10);
}
   ...
   public void caminar(int tiempo) {
    ...
    public void correr(int tiempoDeUso) {
        
        for (Bota bota : botas) {
            bota.usar(2, tiempoDeUso);
        }
    }

    public void propulsar(int tiempoDeUso) {
        for (Guante guantes : guantes) {
            guantes.usar(1, tiempoDeUso);
        }
    }

    public void volar(int tiempoDeUso) {
        for (Guante guantes : guantes) {
            guantes.usar(2, tiempoDeUso);
        }
    }

    public void escribir(String texto) {
        consola.usar(1, texto.length());
    }

    public void hablar(String texto) {
        sintetizador.usar(1, texto.length());
    }

    public void revisarDispositivos() {
        for (Bota bota : botas) {
            while (bota.estaDanado()) {
                bota.reparar();
            }
        }
        for (Guante guante : guantes) {
            while (guante.estaDanado()) {
                guante.reparar();
            }
        }
        while (consola.estaDanado()) {
            consola.reparar();
        }
        while (sintetizador.estaDanado()) {
